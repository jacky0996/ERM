# 多環境支援 (Dev/Uat/Prod) 設定計畫書

為了讓專案能根據不同部署環境（開發、測試、生產）自動切換 API 位址與網域設定，我們採用 Vite 的環境變數機制。
...
2.  **Vite Proxy (僅限開發環境)**：
    在 `vite.config.mts` 中，`proxy` 設定僅在 `dev` 模式下生效。當您執行 `build` 時，Vite 會直接將 `VITE_GLOB_API_URL` 的完整位址寫入編譯後的代碼。
...
1.  **建立 `.env.uat`**：填入 UAT 專用的 API 位址。
2.  **更新指令**：在 `package.json` 加入 `build:uat`。
3.  **建置測試**：嘗試 `pnpm build:uat`，檢查輸出的 `dist/_app.config.js` 檔案中的 `VITE_GLOB_API_URL` 是否正確。
